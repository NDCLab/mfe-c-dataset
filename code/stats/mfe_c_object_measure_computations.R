...
input_organized_path <- paste(proje_wd, "derivatives", "psychopy", "csv_output", "mfe-c-object", sep ="/", collapse = NULL) # input directory for data files generated by the organizer script!
output_path <- paste(proje_wd, "derivatives", "psychopy", "stat_output", "mfe-c-object", sep ="/", collapse = NULL) # output directory
...
main_df <- setNames(data.frame(matrix(ncol = 41, nrow = 0)), c("participant_id", "congAcc", "incongAcc",
                                                               ...
                                                               "reported_errors", "committed_errors", "memoryBias_score", "overall_proRec", "error_proRec",
                                                               "correct_proRec", "num_incong_correctobjects_reported_new", "num_incong_correctobjects_reported_old",
                                                               "num_incong_errorobjects_reported_new", "num_incong_errorobjects_reported_old", "hit_num", "false_alrams_num",
                                                               "miss_num", "corr_rej_num", "incong_error_hit_num",
                                                               "incong_correct_hit_num", "incong_error_miss_num",
                                                               "incong_correct_miss_num",
                                                               "proRec_error_minus_correct",
                                                               "num_of_correct_cong_trials", "num_of_error_cong_trials",
                                                               "num_of_correct_incong_trials", "num_of_error_incong_trials"))
...
num_of_participants_removed_based_on_accuracy <- 0
num_of_participants_removed_based_on_incong_error_num <- 0
num_participants_removed_based_on_num_incong_error_objects_in_memory_surp <- 0
...
for (subject in 1:length(raw_datafiles_list)){
  ...
  number_of_objects_in_surp_memory_task <- nrow(surprise_df)
  surp_rt_threshold <- round(0.375 * number_of_objects_in_surp_memory_task)
    ...
    if (mean(as.numeric(flanker_df$current_trial_accuracy)) >= 0.6){
      ...
      if (surprise_df$num_blocks_excluded[1] <= 3){
        ...
        if (nrow(legit_error_incong_flankerDat) >= 8 ){
          ...
          # Checking to see if there are at least 8 incongruent error objects in the surprise_df of this participant.
          ...
          num_incong_error_objects_in_surp <- 0
          # Counting the number of legit incong error objects available in surprise_df!
          for (rr in 1:nrow(legit_error_incong_flankerDat)){
            temp_object <- legit_error_incong_flankerDat$current_trial_object[rr]
            temp_for_surp <- filter(surprise_df, object == temp_object)
            errorobject_exist_in_surpDat <- ifelse(nrow(temp_for_surp) == 1, 1,0)
            if (errorobject_exist_in_surpDat == 1){
              num_incong_error_objects_in_surp <- num_incong_error_objects_in_surp + 1
            }
          } # Closing the loop that counts the number of incong error objects available in surprise_df!
          if (num_incong_error_objects_in_surp >= 8){
            ...
            num_of_correct_cong_trials <- sum(cong_flankerDat$current_trial_accuracy == 1)
            num_of_error_cong_trials <- sum(cong_flankerDat$current_trial_accuracy == 0)

            num_of_correct_incong_trials <- sum(incong_flankerDat$current_trial_accuracy == 1)
            num_of_error_incong_trials <- sum(incong_flankerDat$current_trial_accuracy == 0)
            ...
            ### number of incong error objects identified as old
            num_incong_errorobjects_reported_old <- 0 # this is the number of incongruent error objects that they report as OLD and will be updated in the loop below:
            num_incong_errorobjects_reported_new <- 0
            for (iii in 1:nrow(error_incong_flankerDat)){
              if (error_incong_flankerDat$current_trial_legitResponse[iii] == 1){ # if this trial is legit
                temp_object_from_flanker <- error_incong_flankerDat$current_trial_object[iii]
                temp_object_row_in_surp <- filter(surprise_df, object == temp_object_from_flanker)
                if (nrow(temp_object_row_in_surp) == 1){ # if old incong error object exist in the surprise_df
                  if (temp_object_row_in_surp$surpAcc == 1){ # the old object is chosen as old correctly
                    num_incong_errorobjects_reported_old <- num_incong_errorobjects_reported_old + 1 # The number of incongruent error objects that they report as OLD
                  } else if (temp_object_row_in_surp$surpAcc == 0){ # the old object is chosen as new incorrectly
                    num_incong_errorobjects_reported_new <- num_incong_errorobjects_reported_new + 1 # The number of incongruent error objects that they report as New
                  }
                }
              }
            ...
            ### number of incong correct objects identified as old
            num_incong_correctobjects_reported_old <- 0 # this is the number of incongruent correct objects that they report as Old and will be updated in the loop below:
            num_incong_correctobjects_reported_new <- 0
            for (iii in 1:nrow(incong_corrDat)){
              if (incong_corrDat$current_trial_legitResponse[iii] == 1){ # if this trial is legit
                temp_object_from_flanker <- incong_corrDat$current_trial_object[iii]
                temp_object_row_in_surp <- filter(surprise_df, object == temp_object_from_flanker)
                if (nrow(temp_object_row_in_surp) == 1){ # if old incong correct object exist in the surprise_df
                  if (temp_object_row_in_surp$surpAcc == 1){ # the old object is chosen as old correctly
                    num_incong_correctobjects_reported_old <- num_incong_correctobjects_reported_old + 1 # The number of incongruent correct objects that they report as Old
                  } else if (temp_object_row_in_surp$surpAcc == 0){ # the old object is chosen as new incorrectly
                    num_incong_correctobjects_reported_new <- num_incong_correctobjects_reported_new + 1 # The number of incongruent correct objects that they report as New
                  }
                }
              }
            ...
            # Remember, in this task, we displayed both new and old images in a given trial so that participants can choose one image as old.
            for (surpTrial in 1:nrow(surprise_df)){
              if (surprise_df$surpAcc[surpTrial] == 1) { # The old object is identified correctly as old
                hit_num <- hit_num + 1
                corr_rej_num <- corr_rej_num + 1
              } else if (surprise_df$surpAcc[surpTrial] == 0){ # The old object is identified incorrectly as new
                miss_num <- miss_num + 1
                false_alrams_num <- false_alrams_num + 1
              }
            } # Closing the loop over surprise_df trials
            # proRec = Proportion Recognized
            overall_proRec <- (hit_num) / ((hit_num) + miss_num) # proRec
            overall_false_alarm_rate <- (false_alrams_num)/ ((false_alrams_num) + (corr_rej_num))

            incong_error_hit_num <- num_incong_errorobjects_reported_old
            incong_error_miss_num <- num_incong_errorobjects_reported_new
            error_proRec <- (incong_error_hit_num) / ((incong_error_hit_num) + incong_error_miss_num) # proRec


            incong_correct_hit_num <- num_incong_correctobjects_reported_old
            incong_correct_miss_num <- num_incong_correctobjects_reported_new
            correct_proRec <- (incong_correct_hit_num) / ((incong_correct_hit_num) + incong_correct_miss_num) # proRec

            proRec_error_minus_correct <- error_proRec - correct_proRec
            ...
            main_df[nrow(main_df) + 1,] <-c(participant_id, congAcc, incongAcc,
                                            ...
                                            miss_num, corr_rej_num, incong_error_hit_num,
                                            incong_correct_hit_num, incong_error_miss_num,
                                            incong_correct_miss_num, proRec_error_minus_correct,
                                            num_of_correct_cong_trials, num_of_error_cong_trials,
                                            num_of_correct_incong_trials, num_of_error_incong_trials)


          } else { # If a participant has been excluded because they had less than 8 legit incong error objects in the surprise memory task, we add 1 to the counter below.
            num_participants_removed_based_on_num_incong_error_objects_in_memory_surp <- num_participants_removed_based_on_num_incong_error_objects_in_memory_surp + 1
            ### Printing output
            print(paste("Participant ", participant_id, " was excluded due to not having 8 legit incong error objects in the surprise memory task."))
