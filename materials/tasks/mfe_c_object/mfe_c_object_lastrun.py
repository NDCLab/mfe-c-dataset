...
for item in files_list:
    ...
    csvFile['old_object_in_surp'] = np.random.permutation(csvFile['old_object_in_surp'].values) # shuffles rows in this column randomly
    csvFile['new_object_in_surp'] = np.random.permutation(csvFile['new_object_in_surp'].values) # shuffles rows in this column randomly
    csvFile['which_side_old_object_displayed'] = np.random.permutation(csvFile['which_side_old_object_displayed'].values) # shuffles rows in this column randomly
...
expName = 'mfe_c_object'  # from the Builder filename that created this script
expInfo = {
    'id': '',
}
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/kihossei/Library/CloudStorage/GoogleDrive-hosseinikianoosh@gmail.com/My Drive/My Digital Life/Professional/Github_Repos/mfe-c-object-dataset/materials/mfe_c_object/mfe_c_object_lastrun.py',
...
thisISI = 0
bigObject = visual.ImageStim(
    win=win,
    name='bigObject', units='deg', 
    image='sin', mask=None, anchor='center',
...
bigObject_2 = visual.ImageStim(
    win=win,
    name='bigObject_2', units='deg', 
    image='sin', mask=None, anchor='center',
    ori=0.0, pos=(0, 0.0), size=(13.59, 9.55),
    color=[1,1,1], colorSpace='rgb', opacity=0.85,
    flipHoriz=False, flipVert=False,
...
instruct_surprise1 = visual.TextStim(win=win, name='instruct_surprise1',
    text='You will now begin a task in which you will be shown two images of objects. Then, you need to choose the object image that you think you have seen in the previous Arrow game (old object).\n\nFor example:\n\nIf the object image on the right side is the one you think you have seen in the Arrow game, press the K key.\n\nIf the object image on the left side is the one you think you have seen in the Arrow game, press the S key.\n\nPlease press the K key to continue.',
    font='Open Sans',
...
text = visual.TextStim(win=win, name='text',
    text='Choose the object you have seen before (old)',
    font='Open Sans',
    units='deg', pos=(0, 10), height=1.5, wrapWidth=None, ori=0.0, 
    color='white', colorSpace='rgb', opacity=None, 
    languageStyle='LTR',
    depth=-1.0);
old_object_image = visual.ImageStim(
    win=win,
    name='old_object_image', units='deg', 
    image='sin', mask=None, anchor='center',
    ori=0.0, pos=[0,0], size=(13.59, 9.55),
    color=[1,1,1], colorSpace='rgb', opacity=None,
    flipHoriz=False, flipVert=False,
    texRes=128.0, interpolate=True, depth=-2.0)
new_object_image = visual.ImageStim(
    win=win,
    name='new_object_image', units='deg', 
    image='sin', mask=None, anchor='center',
    ori=0.0, pos=[0,0], size=(13.59, 9.55),
    color=[1,1,1], colorSpace='rgb', opacity=None,
    flipHoriz=False, flipVert=False,
...
for thisPrac_block_loop in prac_block_loop:
    ...
    for thisPrac_trial_loop in prac_trial_loop:
        ...
        bigObject.setImage(straightObj)
        ...
        # keep track of which components have finished
        prac_stimRoutineComponents = [bigObject, cover_background_2, prac_centerImg, prac_rightImg1, prac_rightImg2, prac_leftImg1, prac_leftImg2, prac_fixImg, prac_stim_keyResp]
        ...
        while continueRoutine:
            ...
            # *bigObject* updates
            if bigObject.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                bigObject.frameNStart = frameN  # exact frame index
                bigObject.tStart = t  # local t and not account for scr refresh
                bigObject.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(bigObject, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'bigObject.started')
                bigObject.setAutoDraw(True)
            if bigObject.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > bigObject.tStartRefresh + thisISI-frameTolerance:
                    # keep track of stop time/frame for later
                    bigObject.tStop = t  # not accounting for scr refresh
                    bigObject.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'bigObject.stopped')
                    bigObject.setAutoDraw(False)
...
for thisTask_block_loop in task_block_loop:
    ...
    for thisTask_trial_loop in task_trial_loop:
        ...
        bigObject_2.setImage(straightObj)
        ...
        # keep track of which components have finished
        task_stimRoutineComponents = [bigObject_2, cover_background, task_centerImg, task_rightImg1, task_rightImg2, task_leftImg1, task_leftImg2, task_fixImg, task1_stim_keyResp]
        ...
        while continueRoutine:
            ...
            # *bigObject_2* updates
            if bigObject_2.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                bigObject_2.frameNStart = frameN  # exact frame index
                bigObject_2.tStart = t  # local t and not account for scr refresh
                bigObject_2.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(bigObject_2, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'bigObject_2.started')
                bigObject_2.setAutoDraw(True)
            if bigObject_2.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > bigObject_2.tStartRefresh + thisISI-frameTolerance:
                    # keep track of stop time/frame for later
                    bigObject_2.tStop = t  # not accounting for scr refresh
                    bigObject_2.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'bigObject_2.stopped')
                    bigObject_2.setAutoDraw(False)
...
for thisSurprise_block_loop in surprise_block_loop:
    ...
    for thisTrial in trials:
        ...
        if which_side_old_object_displayed == 'right':
            old_img_position = [13.3, 1]
            new_img_position = [-13.3, 1]
        elif which_side_old_object_displayed == 'left':
            old_img_position = [-13.3, 1]
            new_img_position = [13.3, 1]
            
        old_object_image.setPos(old_img_position)
        old_object_image.setImage(old_object_in_surp)
        new_object_image.setPos(new_img_position)
        new_object_image.setImage(new_object_in_surp)
        ...
        # keep track of which components have finished
        surpriseTaskComponents = [text, old_object_image, new_object_image, instructsurpA1_right, instructsurpA2_left, surprise_key_resp]
        ...
        while continueRoutine:
            ...
            # *old_object_image* updates
            if old_object_image.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                old_object_image.frameNStart = frameN  # exact frame index
                old_object_image.tStart = t  # local t and not account for scr refresh
                old_object_image.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(old_object_image, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'old_object_image.started')
                old_object_image.setAutoDraw(True)
            
            # *new_object_image* updates
            if new_object_image.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                new_object_image.frameNStart = frameN  # exact frame index
                new_object_image.tStart = t  # local t and not account for scr refresh
                new_object_image.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(new_object_image, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'new_object_image.started')
                new_object_image.setAutoDraw(True)
